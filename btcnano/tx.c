...
static void push_tx_input(const struct btcnano_tx_input *input,
...
static void push_tx_output(const struct btcnano_tx_output *output,
			  void (*push)(const void *, size_t, void *), void *pushp)
{
...
static bool uses_witness(const struct btcnano_tx *tx)
{
	size_t i;
 ...
 * items for the txin.  */
static void push_witnesses(const struct btcnano_tx *tx,
			  void (*push)(const void *, size_t, void *), void *pushp)
{
	size_t i;
	for (i = 0; i < tal_count(tx->input); i++) {
...
static void push_tx(const struct btcnano_tx *tx,
		   void (*push)(const void *, size_t, void *), void *pushp,
		   bool bip144)
{
	varint_t i;
...
static void hash_prevouts(struct sha256_double *h, const struct btcnano_tx *tx)
{
...
static void hash_sequence(struct sha256_double *h, const struct btcnano_tx *tx)
{
	struct sha256_ctx ctx;
	size_t i;
 ...
 * script); */
static void hash_outputs(struct sha256_double *h, const struct btcnano_tx *tx)
{
	struct sha256_ctx ctx;
	size_t i;
...
{
	struct btcnano_tx *tx = tal(ctx, struct btcnano_tx);
	size_t i;

	tx->output = tal_arrz(tx, struct btcnano_tx_output, output_count);
	tx->input = tal_arrz(tx, struct btcnano_tx_input, input_count);
	for (i = 0; i < tal_count(tx->input); i++) {
...
static void pull_witness(struct btcnano_tx_input *inputs, size_t i,
					...
					size_t *max, struct btcnano_tx *tx)
{
	...
	tx->input = tal_arr(tx, struct btcnano_tx_input, count);
	for (i = 0; i < tal_count(tx->input); i++)
		pull_input(tx, cursor, max, tx->input + i);

	count = pull_length(cursor, max, 8 + 1);
	tx->output = tal_arr(tx, struct btcnano_tx_output, count);
	for (i = 0; i < tal_count(tx->output); i++)
...
{
	struct btcnano_tx *tx = tal(ctx, struct btcnano_tx);
	return pull_btcnano_tx_onto(ctx, cursor, max, tx);
}
...
/* <sigh>.  Btcnano represents hashes as little-endian for RPC. */
...
static char *fmt_btcnano_tx(const tal_t *ctx, const struct btcnano_tx *tx)
{
...
static char *fmt_btcnano_txid(const tal_t *ctx, const struct btcnano_txid *txid)
{
	char *hexstr = tal_arr(ctx, char, hex_str_size(sizeof(*txid)));

	btcnano_txid_to_hex(txid, hexstr, hex_str_size(sizeof(*txid)));
	return hexstr;
}

REGISTER_TYPE_TO_STRING(btcnano_tx, fmt_btcnano_tx);
REGISTER_TYPE_TO_STRING(btcnano_txid, fmt_btcnano_txid);
