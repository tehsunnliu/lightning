#ifndef LIGHTNING_BTCNANO_SCRIPT_H
#define LIGHTNING_BTCNANO_SCRIPT_H
#include "config.h"
#include "signature.h"
#include "tx.h"
...
u8 *btcnano_redeem_2of2(const tal_t *ctx,	const struct pubkey *key1,	const struct pubkey *key2);
...
void btcnano_witness_p2sh_p2wpkh(const tal_t *ctx, struct btcnano_tx_input *input, const secp256k1_ecdsa_signature *sig, const struct pubkey *key);
...
u8 *scriptpubkey_witness_raw(const tal_t *ctx, u8 version, const u8 *wprog, size_t wprog_size);
...
u8 *btcnano_wscript_to_local(const tal_t *ctx,
			     u16 to_self_delay,
...
u8 *btcnano_wscript_htlc_tx(const tal_t *ctx, u16 to_self_delay, const struct pubkey *revocation_pubkey, const struct pubkey *local_delayedkey);
...
#define BTCNANO_SCRIPTPUBKEY_P2PKH_LEN (1 + 1 + 1 + 20 + 1 + 1)

/* OP_HASH160 + PUSH(20-byte-hash) + OP_EQUAL */
#define BTCNANO_SCRIPTPUBKEY_P2SH_LEN (1 + 1 + 20 + 1)

/* OP_0 + PUSH(20-byte-hash) */
#define BTCNANO_SCRIPTPUBKEY_P2WPKH_LEN (1 + 1 + 20)

/* OP_0 + PUSH(32-byte-hash) */
#define BTCNANO_SCRIPTPUBKEY_P2WSH_LEN (1 + 1 + 32)

#endif /* LIGHTNING_BTCNANO_SCRIPT_H */
