...
/* Btcnano's OP_HASH160 is RIPEMD(SHA256()) */
...
/* Btcnano script stack values are a special, special snowflake.
 *
...
u8 *btcnano_redeem_2of2(const tal_t *ctx,
...
u8 *scriptpubkey_p2pkh(const tal_t *ctx, const struct btcnano_address *addr)
{
	u8 *script = tal_arr(ctx, u8, 0);
...
u8 *btcnano_redeem_p2pkh(const tal_t *ctx, const struct pubkey *pubkey,
...
u8 *btcnano_redeem_p2sh_p2wpkh(const tal_t *ctx, const struct pubkey *key)
{
...
u8 *btcnano_scriptsig_p2sh_p2wpkh(const tal_t *ctx, const struct pubkey *key)
{
	u8 *redeemscript = btcnano_redeem_p2sh_p2wpkh(ctx, key), *script;
...
void btcnano_witness_p2sh_p2wpkh(const tal_t *ctx,
				 struct btcnano_tx_input *input,
				 const secp256k1_ecdsa_signature *sig,
				 const struct pubkey *key)
{
	input->script = btcnano_scriptsig_p2sh_p2wpkh(ctx, key);
	input->witness = btcnano_witness_p2wpkh(ctx, sig, key);
}

u8 **btcnano_witness_p2wpkh(const tal_t *ctx,
			    const secp256k1_ecdsa_signature *sig,
			    const struct pubkey *key)
{
	u8 **witness;
...
u8 **btcnano_witness_2of2(const tal_t *ctx,
			  ...
			  const struct pubkey *key2)
{
	...
	witness[3] = btcnano_redeem_2of2(witness, key1, key2);
	return witness;
}
...
bool is_p2pkh(const u8 *script, struct btcnano_address *addr)
{
	size_t script_len = tal_len(script);

	if (script_len != BTCNANO_SCRIPTPUBKEY_P2PKH_LEN)
		return false;
	if (script[0] != OP_DUP)
		return false;
...
bool is_p2sh(const u8 *script, struct ripemd160 *addr)
{
	...
	if (script_len != BTCNANO_SCRIPTPUBKEY_P2SH_LEN)
		return false;
...
bool is_p2wsh(const u8 *script, struct sha256 *addr)
{
	...
	if (script_len != BTCNANO_SCRIPTPUBKEY_P2WSH_LEN)
		return false;
	if (script[0] != OP_0)
		return false;
...
bool is_p2wpkh(const u8 *script, struct btcnano_address *addr)
{
	size_t script_len = tal_len(script);

	if (script_len != BTCNANO_SCRIPTPUBKEY_P2WPKH_LEN)
		return false;
	if (script[0] != OP_0)
		return false;
...
u8 **btcnano_witness_sig_and_element(const tal_t *ctx,
				     const secp256k1_ecdsa_signature *sig,
 ...
 *     OP_CHECKSIG
 */
u8 *btcnano_wscript_to_local(const tal_t *ctx, u16 to_self_delay,
...
u8 **btcnano_to_local_spend_revocation(const tal_t *ctx,
 ...
 *     OP_ENDIF
 */
u8 *btcnano_wscript_htlc_offer_ripemd160(const tal_t *ctx,
...
u8 *btcnano_wscript_htlc_offer(const tal_t *ctx,
			       const struct pubkey *localhtlckey,
			       const struct pubkey *remotehtlckey,
			       ...
			       const struct pubkey *revocationkey)
{
	...
	return btcnano_wscript_htlc_offer_ripemd160(ctx, localhtlckey,
						    remotehtlckey,
 ...
 *     OP_ENDIF
 */
u8 *btcnano_wscript_htlc_receive_ripemd(const tal_t *ctx,
...
u8 *btcnano_wscript_htlc_receive(const tal_t *ctx,
				 const struct abs_locktime *htlc_abstimeout,
				 const struct pubkey *localhtlckey,
				 const struct pubkey *remotehtlckey,
				 const struct sha256 *payment_hash,
				 const struct pubkey *revocationkey)
{
	...
	ripemd160(&ripemd, payment_hash->u.u8, sizeof(payment_hash->u));
	return btcnano_wscript_htlc_receive_ripemd(ctx, htlc_abstimeout,
 ...
 */
u8 **btcnano_witness_htlc_timeout_tx(const tal_t *ctx,
...
u8 **btcnano_witness_htlc_success_tx(const tal_t *ctx,
				     const secp256k1_ecdsa_signature *localhtlcsig,
...
u8 *btcnano_wscript_htlc_tx(const tal_t *ctx,
			    u16 to_self_delay,
			    const struct pubkey *revocation_pubkey,
			    const struct pubkey *local_delayedkey)
{
