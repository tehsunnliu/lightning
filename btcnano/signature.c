...
#else
static void dump_tx(const char *msg,
		    const struct btcnano_tx *tx, size_t inputnum,
		    const u8 *script,
		    const struct pubkey *key,
		    const struct sha256_double *h)
{
	       ...
	       secp256k1_ecdsa_signature *s)
{
	...
	ok = secp256k1_ecdsa_sign(secp256k1_ctx, s, h->sha.u.u8, privkey->secret.data, NULL, NULL);
	assert(ok);
}

/* Only does SIGHASH_ALL */
static void sha256_tx_one_input(struct btcnano_tx *tx,
				size_t input_num,
				const u8 *script,
...
/* Only does SIGHASH_ALL */
void sign_tx_input(struct btcnano_tx *tx,
		   unsigned int in,
		   const u8 *subscript,
		   const u8 *witness_script,
...
{
	int ret;
	ret = secp256k1_ecdsa_verify(secp256k1_ctx, signature, hash->sha.u.u8, &key->pubkey);
	return ret == 1;
}

bool check_tx_sig(struct btcnano_tx *tx, 
		  size_t input_num,
